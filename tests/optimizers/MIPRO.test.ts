...
import { MIPRO } from "../../src/optimizers/MIPRO.js";
...
describe("MIPRO", () => {
  ...
  it("combines instruction and demo optimization", async () => {
    ...
    expect(optimized).toBeDefined();
    // MIPRO runs both COPRO and BootstrapFewShotWithRandomSearch phases,
    // so the result should be a valid optimized module.
    expect(optimized).not.toBe(student);
  });
});
