...
import { AvatarOptimizer } from "../../src/optimizers/AvatarOptimizer.js";
...
describe("AvatarOptimizer", () => {
  beforeEach(() => settings.reset());

  it("throws when no LM is configured", async () => {
    const optimizer = new AvatarOptimizer({ numAvatars: 1 });
    await expect(
  ...
  it("compiles and returns a Module", async () => {
    ...
    const optimizer = new AvatarOptimizer({ numAvatars: 1, maxLabeledDemos: 2 });
