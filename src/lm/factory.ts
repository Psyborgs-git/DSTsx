...
LM.registerProvider("lmstudio", (model, opts) => new LMStudio({ ...(model ? { model } : {}), ...(opts.baseURL ? { baseURL: opts.baseURL } : {}) }));
LM.registerProvider("huggingface", (model, opts) => new HuggingFace({ ...(model ? { model } : {}), ...(opts.apiKey ? { apiKey: opts.apiKey } : {}) }));
LM.registerProvider("hf", (model, opts) => new HuggingFace({ ...(model ? { model } : {}), ...(opts.apiKey ? { apiKey: opts.apiKey } : {}) }));
